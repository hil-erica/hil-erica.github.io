...
function addUserInterface(username){
	var isLogin = false;
	if(loginUsers.includes(username))isLogin = true;
	userCounter ++;
...
async function callButtonClicked(){
	...
	var remoteUserId = userInput.value;
	if(callButton.innerHTML.indexOf('Call') >= 0){
		if(await callRemoteOne(remoteUserId)){
			//callButton.innerHTML="Close";
			updateConnectUI(remoteUserId, true);
		} else {
			//console.log("failed to callRemoteOne");
		}
	}else {
		if(await closeRemote(remoteUserId)){
		} else {
			//console.log("failed to closeRemote");
		}
		//callButton.innerHTML="Call";
		updateConnectUI(remoteUserId, false);
	}
...
function updateUserConnect(userCounter, connected){
	var callButton = userFrame.$('#call_'+userCounter);
	var removeButton = userFrame.$('#remove_'+userCounter);
	var userInput = userFrame.$('#username_'+userCounter);
...
function updateConnectUI(remoteUserId, connected){
	console.log(remoteUserId +' to connected:' + connected);
	var foundUse = false;
	for(var i= 0; i < chatUserList.length; i++){
		var userCounter = chatUserList[i].getAttribute("userCounter");
		var callButton = userFrame.$('#call_'+userCounter);
		...
		if(userInput.value == remoteUserId){
			updateUserConnect(userCounter, connected);
			foundUse = true;
			break;
		}
...
function connectClick(remoteUserId, action){
	for(var i= 0; i < chatUserList.length; i++){
		var userCounter = chatUserList[i].getAttribute("userCounter");
		var callButton = userFrame.$('#call_'+userCounter);
		var removeButton = userFrame.$('#remove_'+userCounter);
	...
	//ないので追加
	console.log("create button of "+remoteUserId);
	//var divElement = addUserInterface(remoteUserId);
	//var userCounter = divElement.getAttribute("userCounter");
	updateConnectUI(remoteUserId, false);
	connectClick(remoteUserId, action);
